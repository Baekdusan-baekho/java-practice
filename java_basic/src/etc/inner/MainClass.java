...
public class MainClass {
	...
	//static -> 메모리 효율을 높이기 위해 선언. 
	//(바깥쪽 클래스 호출 시마다 재생성하는 것을 방지)
...
//		System.out.println(sub3);
		...
		System.out.println(multiCal.operate(400, 2));
		
		
		// 나눗셈 하는 계산기 (람다) -> 인터페이스의 추상메서드가 딱 1개일 경우에만 가능.
		// 어짜피 인터페이스에 추상메서드가 1개이기 때문에 이름을 따로 명시하지 않음.
		// 메서드 내부 로직이 단 한 줄이라면 괄호 생략 가능 -> return 값으로 취급.
		Calculator divCal = (n1, n2) -> n1 / n2;
		System.out.println(divCal.operate(250, 5));
