...
public class MultiCatchExample {
	...
	public static void main(String[] args) {
		...
		}catch(InputMismatchException | ArithmeticException e) { //exception e 모든 예외 받음
			System.out.println("똑바로 입력해라 임마~!");
		}catch(ArrayIndexOutOfBoundsException e) {
			System.out.println("인덱스 범위를 벗어났어요!");
		}catch(Exception e) { //예측못한 에러가 발생했을 경우
			System.out.println("예측하지 못한 에러 발생!");
			System.out.println("에러 원인: " + e.getMessage()); 
			//예외 발생이유 매개변수 이름.getmessage
			//exception 에러가 가장 위에 있으면 모든 에러를 처리해서
			//여러가지 예외처리가 할 일이 없다.
