		...
		while(true) {
            ...
            if(menu == 1) {
            	for(int i=0; i<userNums.length;i++) {
            		...
            		outer: for(int f=0; f<userNums.length;f++) {
            			if(aa.equals(userNums[f])) {
            				System.out.println("사번이 중복되었습니다 다른 번호를 입력십시오.");
            				System.out.println(f);
            				String aaa = sc.next();
            				for(int ff=0; ff<userNums.length;ff++) {
            				if(aaa.equals(userNums[i])) {flag=false;}
            				else {flag=true;}
            				if(flag) {
            				userNums[i] = aaa;
            				aa=aaa;
            		...
            		System.out.printf("사번: %s\n이름: %s\n나이: %d\n부서명: %s\n",
            				userNums[i], names[i]=bb, ages[i], departments[i]);
            		System.out.println("--------------------------------------------------");
            		System.out.println(Arrays.toString(userNums));
            		break;
            	}
            	
            	System.out.println("사원 정보 신규 등록이 완료되었습니다.\n\n\n\n");
            	System.out.println(Arrays.toString(userNums));
