...
import etc.api.util.arrays.AraaysExample;
...
public class ArrayListExample {
	...
	public static void main(String[] args) {
        ...
        */
		...
		List<String> nick = new ArrayList<>(); //<> 타입 고정
...
//		System.out.println(!nick.isEmpty()); 리스트가 차있는지 여부
...
//		List<int> score = new ArrayList<>(); 
		//int는 기본 데이터 타입으로 받을 수 없다 객체로 받아야 한다.
		...
		//오름차 정렬 (작은값부터 순서대로 커지는 정렬) collections에서 제공
		Collections.sort(score);
		System.out.println(score);
		
		//내림차 정렬 (큰 값부터 순서대로 작아지는 정렬)
//		Collections.reverse(score); 오름차 후에 반대로 함 원래배열의 반대로 정렬
